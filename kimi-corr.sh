...
# mdrun com GPU se disponivel
# CORRECAO: desabilita PME GPU para minimizacao de energia (steep/cg)
mdrun() {
    local pme_flag="-pme gpu"
    # Verifica se eh minimizacao de energia pelo prefixo (-deffnm)
    for arg in "$@"; do
        if [[ "$arg" == "-deffnm" ]]; then
            local next_idx=$((i+1))
            # O proximo argumento eh o nome do deffnm
            :
        fi
        if [[ "${prev_arg:-}" == "-deffnm" && "$arg" == "em" ]]; then
            pme_flag=""  # Desabilita PME GPU para EM
        fi
        prev_arg="$arg"
    done
    # Alternativa mais simples: verifica diretamente se 'em' esta nos argumentos
    local is_em="no"
    for arg in "$@"; do
        if [[ "$arg" == "em" ]]; then
            is_em="yes"
    ...
    if [[ "$GMX" == *"gmx_mpi"* ]]; then
        mpirun --use-hwthread-cpus -np 1 "$GMX" mdrun \
            -ntomp "$NTOMP" -nb gpu $pme_flag -bonded gpu \
            -gpu_id "$GPU_ID" -pin on "$@"
    else
        "$GMX" mdrun -ntomp "$NTOMP" -nb gpu $pme_flag -bonded gpu \
            -gpu_id "$GPU_ID" -pin on "$@"
    fi
}
